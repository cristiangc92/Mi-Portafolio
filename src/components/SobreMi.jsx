import "../style/SobreMi.css";

const SobreMi = () => {
  return (
    <div id="sobreMi">
      <h1 className="tituloSobreMi">SOBRE MÍ</h1>
      <div className="parrafo">
        <p>
          Soy un apasionado de la tecnología y en especial de la programación.
          Me considero una persona curiosa, autodidacta, con buenas habilidades
          sociales y de comunicación. Estoy en búsqueda de una oportunidad que
          me permita continuar desarrollando mis habilidades profesionales y
          personales en un ámbito de trabajo real. <br />
          <br />
          <b>
            ¡Para conocer un poco más de mí, te invito a continuar scrolleando!
          </b>
        </p>
      </div>
    </div>
  );
};

export default SobreMi;
